...
public class FormController implements Initializable {
    ...
    Color generalDopColor = Color.BLACK;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        addTrainPanel.setOnDragDropped(new EventHandler<DragEvent>() {
            public void handle(DragEvent event) {
                ...
                if (db.hasString()) {
                    switch (db.getString()) {
                        case "Локомотив":
                            train = new Train(100, 4, 100, 1000, generalColor);
                            inter = train;
                            ...
                            break;
                        case "Электровоз":
                            PasTrain pasTrain = new PasTrain(100, 4, 100, 1000, generalColor, true, false, true, generalDopColor);
                            inter = pasTrain;
        ...
        mainColorID.setOnDragDropped(new EventHandler<DragEvent>() {
            public void handle(DragEvent event) {
                ...
                if (db.hasString()) {
                    ...
                    inter.setMainColor(generalColor);
                    inter.setPosition(50, 50);
                    addTrainPanel.getChildren().removeAll();
                    addTrainPanel.getChildren().setAll(inter.drawCar());
        ...
        dopColorID.setOnDragOver(new EventHandler<DragEvent>() {
            public void handle(DragEvent event) {
                if (event.getGestureSource() != dopColorID &&
                        event.getDragboard().hasString()) {
                    event.acceptTransferModes(TransferMode.COPY_OR_MOVE);
                }
                event.consume();
        ...
        dopColorID.setOnDragExited(new EventHandler<DragEvent>() {
            public void handle(DragEvent t) {
                dopColorID.setEffect(null);
                t.consume();
            }
        });
        dopColorID.setOnDragDropped(new EventHandler<DragEvent>() {
            public void handle(DragEvent event) {
                Dragboard db = event.getDragboard();
                boolean success = false;
                if (db.hasString()) {
                    generalDopColor = Color.web(db.getString());
                    inter.setDopColor(generalDopColor);
                    inter.setPosition(50, 50);
                    addTrainPanel.getChildren().removeAll();
                    addTrainPanel.getChildren().setAll(inter.drawCar());
                    System.out.println(db.getString());
